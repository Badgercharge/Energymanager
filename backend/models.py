...
class ChargePointState:
    ...
    # Aktueller Modus und Soll-Leistung (kW)
    mode: Mode = "eco"
    target_kw: float = 0.0

    # Elektrische Parameter (für Umrechnung kW <-> A)
    phase_count: int = 3
    voltage_per_phase: float = 230.0
    max_current_a: float = 16.0

    # Status/Fehler laut StatusNotification
    ...
    session_start_kwh_reg: Optional[float] = None
    session_kwh: Optional[float] = None
    session_est_end_at: Optional[datetime] = None
