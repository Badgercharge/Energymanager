...
class CentralSystem(CP):
    ...
    async def on_status(self, status, error_code=None, **kwargs):
        ...
        if st:
            ...
            # Default bei Start eines Ladevorgangs:
            # Wenn in "Charging" gewechselt wird, setze Standard: ECO + Boost 100% bis 07:00, Reset Mail-Flag
    ...
    async def on_start_tx(self, connector_id, id_tag, meter_start, **kwargs):
        # Auch hier Default setzen – doppelt hält besser (nicht alle Boxen schicken status=Charging zuverlässig)
        st = STATE.get(self.id)
        if st:
            st.mode = "eco"
            st.boost_enabled = True
